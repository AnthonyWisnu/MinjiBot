...
// Keep backward-compat alias for existing code that imports DownloadedVideo
...
export class DownloaderService {
  /**
   * Download TikTok video (unchanged).
   */
  async downloadVideo(url: string, kind: DownloaderKind): Promise<DownloadedVideo> {
    ...
    try {
      await this.runDownloaderArgs(url, rawOutputTemplate, kind)
  ...
  /**
   * Download Instagram content — handles reel, photo, story, and carousel.
   * Returns single item or multi items (carousel).
   */
  ...
  private async runDownloaderArgs(
    url: string,
