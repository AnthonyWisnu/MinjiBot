...
export class TenantPanelService {
  ...
  async renderPanel(context: CommandContext): Promise<PanelResult> {
    ...
    const lines = [
      ...
      "",
      "👥 *TIM PENGELOLA*",
      `• *Owner*: ${tenantGroup.ownerJid ? `@${tenantGroup.ownerJid.split("@")[0] ?? ""}` : "-"}`,
      `• *Admin*: ${this.formatAdminList(tenantAdmins.map((a) => a.userJid))}`,
      "",
      "🛡️ *MODERASI & KEAMANAN*",
      ...
      "",
      "📢 *NOTIFIKASI*",
      `• *Welcome*: ${this.fmtBool(feature.welcomeEnabled)}`,
      `• *Goodbye*: ${this.fmtBool(feature.goodbyeEnabled)}`,
      "",
      "🎮 *UTILITAS & FITUR*",
      `• *Game & Kuis*: ${this.fmtBool(feature.gameEnabled)}`,
      `• *Downloader*: ${this.fmtBool(feature.downloaderEnabled)}`,
      `• *AI Photo*: ${this.fmtBool(feature.hdEnabled)}`,
